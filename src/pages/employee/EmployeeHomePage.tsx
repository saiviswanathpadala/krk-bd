...
interface StatCardProps {
  ...
  metric1Label: string;
  metric1Value: number;
  metric2Label?: string; // Made optional
  metric2Value?: number; // Made optional
  color: string;
  buttonText: string;
  onPress: () => void;
}
    ...
    <div className="h-full bg-card rounded-xl shadow-md border border-border/50 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
      <div className="h-full p-4 sm:p-5 flex flex-col">
        ...
        <div className="flex-1 mb-3 sm:mb-4">
          {metric2Label && metric2Value !== undefined ? (
            ...
            <div className="h-full">
              <div 
                className="h-full p-4 sm:p-5 rounded-lg flex flex-col items-center justify-center transition-transform hover:scale-105 duration-200"
                style={{ backgroundColor: color + '15' }}
              >
                <p className="text-xs sm:text-sm font-medium text-muted-foreground mb-1.5 text-center">{metric1Label}</p>
                <p className="text-2xl sm:text-3xl font-bold text-foreground">{formatNumber(metric1Value)}</p>
              </div>
            </div>
          )}
        </div>
...
const EmployeeWelcomeSection: React.FC<{ userName: string; lastUpdated?: string }> = ({ userName, lastUpdated }) => {
...
export const EmployeeHomePage: React.FC = () => {
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useQuery<EmployeeDashboardStats>({
    queryKey: ['employee-dashboard-stats', user?.id],
    queryFn: async () => {
      const response = await employeeAPI.getDashboardStats(token!);
  ...
  if (isLoading && !stats) {
    return (
      <div className="min-h-screen pb-8 px-4 sm:px-6 lg:px-8">
        <EmployeeWelcomeSection userName={user?.name || 'Employee'} />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6 auto-rows-fr">
          ...
          <StatCardSkeleton hasMetric2={true} />
          <StatCardSkeleton hasMetric2={false} />
        </div>
      </div>
    );
  }
    ...
    ? [
        {
          ...
          metric1Label: 'Assigned Properties',
          metric1Value: stats.properties.totalAssigned,
          metric2Label: 'Under Review',
          metric2Value: stats.properties.underReview,
          color: '#0a66c2', // Navy blue - matches mobile theme.colors.primary.navy
          buttonText: 'View Details',
          onPress: () => navigate('/employee/properties'),
        },
        {
          id: 'banners',
          title: 'Banners',
          ...
          metric2Label: 'Under Review',
          metric2Value: stats.banners.underReview,
          color: '#00a699', // Teal - matches mobile theme.colors.primary.olive
          buttonText: 'View Details',
          onPress: () => navigate('/employee/banners'),
        },
        {
          id: 'agents',
          title: 'Agents',
          icon: <Users className="w-5 h-5" />,
          metric1Label: 'Assigned Agents',
          metric1Value: stats.agents.totalAssigned,
          color: '#A855F7', // Purple - matches mobile
          buttonText: 'View Details',
          onPress: () => navigate('/employee/agents'),
        },
      ]
    : [];
  ...
  return (
    <div className="min-h-screen pb-8 px-4 sm:px-6 lg:px-8">
      <EmployeeWelcomeSection userName={user?.name || 'Employee'} lastUpdated={stats?.lastUpdated} />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6 auto-rows-fr">
